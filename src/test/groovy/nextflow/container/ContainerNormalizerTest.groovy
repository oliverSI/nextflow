 ...
 */
class ContainerNormalizerTest extends Specification {

    def 'test docker is absolute image name' () {

        expect:
        ContainerNormalizer.isAbsoluteDockerName(image) == expected
    ...
    def 'test normalize docker image name' () {
        ...
        given:
        def n = new ContainerNormalizer([registry: registry])
    ...
    def 'test singularity relative path exists' () {
        ...
        def n = new ContainerNormalizer([:], base.toAbsolutePath().toString())
    ...
    def 'test normalize method for docker' () {
        given:
        def n = Spy(ContainerNormalizer,constructorArgs:[[engine: 'docker', enabled: true, registry: registry]])
    ...
    def 'test normalize method for shifter' () {
        ...
        given:
        def n = Spy(ContainerNormalizer,constructorArgs:[[engine: 'shifter', enabled: true]])
    ...
    def 'test normalize method for singularity' () {
        given:
        def n = Spy(ContainerNormalizer,constructorArgs:[[engine: 'singularity', enabled: true]])
